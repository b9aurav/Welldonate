...
const MenuItem = ({
  ...
  <div
    className={`w-[215px] h-[48px] rounded-[15px] ${
      isActive && isActive === name && "bg-[#F8F9FA] shadow-lg"
  ...
  >
    {!isActive ? (
      <div className={`w-1/4 h-1/2`}>{icon}</div>
    ) : (
      <div
        className={`w-1/4 h-1/2 text-[#343A40] ${
...
const Sidebar = (props: Props) => {
  ...
  const { address, logout } = useStateContext() as StateContextType;
